...
function getSectionByTitle(title) {
    const sections = getMetadataStructure().filter(s => s.concepts[s.main] === title)
    if (!sections.length) {
...
function getSectionById(sectionId) {
    const sections = getMetadataStructure().filter(s => s.main === sectionId)
    if (!sections.length) {
        throw `ERROR: Unexpected section id: "${sectionId}"`
    }
    return sections[0]
}
...
function getConceptNameById(sectionId, conceptId) {
    return getSectionById(sectionId).concepts[conceptId]
}

function getSectionIds() {
    return getMetadataStructure().map(s => s.main)
}

function getConceptIdsBySectionId(sectionId) {
    const concepts = getSectionById(sectionId).concepts
    return Object.keys(concepts).sort((a,b) => concepts[a] - concepts[b])
}

function getSectionNameById(sectionId) {
    return getSectionById(sectionId).concepts[sectionId]
}
