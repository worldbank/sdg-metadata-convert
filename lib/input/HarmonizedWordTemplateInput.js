    ...
    read(source, descriptors) {
        ...
        return mammoth.convertToHtml({ path: source }, mammothOptions).then(result => {
            ...
            $('body > *').each((idx, element) => {
                ...
                if (text === '' && lastConcept === 'OTHER_DOC') {
                    // Special case - skip blank paragraphs in the last section.
                    // Do nothing, but allow to continue to next in loop.
                }
                else if (text === 'Definitions of Metadata Concepts' && lastConcept === 'OTHER_DOC') {
                    // Then abort completely if we see the beginning of the definitions section.
                    return false
                }
                else {
                    try {
