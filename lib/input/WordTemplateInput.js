 ...
 *   Microsoft Word tends to have a lot of non-breaking and/or extra spaces.
 *   This setting converts them all into a single regular space.
 * incompatibleImageTypes: array of string
 *   Certain image types can cause memory issues if they are too big. This
 *   defaults to ['tiff', 'x-emf'] if not specified.
 * cheerio: object
 *   Optionally specify any options to be passed in when the Cheerio
 *   library is parsing the HTML. For available options see:
 *   https://cheerio.js.org/interfaces/cheeriooptions.html
 */
class WordTemplateInput extends BaseInput {

    read(source) {
        const mammothOptions = this.getMammothOptions()
        const cheerioOptions = this.getCheerioOptions()
    ...
    unescapeMath(html) {
        ...
        let matches = matchAll(html, regexp)
    ...
    getCheerioOptions() {
        return (this.options.cheerio) ? this.options.cheerio : null
    }
    ...
    parseConcepts(table, $) {
        ...
        $(table).find('> tbody > tr').slice(2).each((idx, conceptRow) => {
            ...
            if (this.isConceptValueValid(conceptValueCell, $)) {
                const context = {conceptName: conceptName}
                try {
                    const conceptId = conceptStore.getConceptIdByName(conceptName)
                ...
                let conceptValue = this.prepareOutput(conceptValueCell, $, context)
    ...
    isConceptValueValid(input, $) {
        // Catch the special case of image-only content.
        if ($(input).prop('tagName') === 'IMG' || $(input).find('img').length > 0) {
            return true
        }
    ...
    prepareOutput(input, $, context) {
        // Special treatment of some concepts.
        if (context.conceptId === 'META_LAST_UPDATE' || context.conceptName === '0.e. Metadata update') {
    ...
    rawHtmlPreparation(rawHtml, messages) {
        let prepped = rawHtml
        prepped = this.unescapeMath(prepped)
        if (this.options.normalizeWhitespace) {
            prepped = prepped.replace(/\s+/g, ' ')
        }
