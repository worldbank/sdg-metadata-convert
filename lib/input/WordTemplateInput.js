...
class WordTemplateInput extends BaseInput {
    ...
    removeIncompatibleImages($) {
        const incompatibleTypes = this.getIncompatibleImageTypes()
        const removeImages = []
        const imageTypesRemoved = []
        $('img').each((idx, img) => {
            const src = $(img).attr('src')
            const imgType = src.split(';base64')[0].replace('data:image/', '')
            if (incompatibleTypes.includes(imgType)) {
                removeImages.push(img)
                imageTypesRemoved.push(imgType)
            }
        })
        removeImages.forEach(img => $(img).remove())
        return imageTypesRemoved
    }
