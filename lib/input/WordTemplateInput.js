...
class WordTemplateInput extends BaseInput {
    ...
    read(source) {
        ...
        return mammoth.convertToHtml({ path: source }, mammothOptions).then(result => {
            ...
            const seriesIncludeOthers = info.nextAll(':contains(Include all other series in the same indicators)').first().next().text() === 'Yes'
            ...
            if (!seriesIncludeOthers) {
                return new Metadata(concepts, descriptors, messages)
            }
            else {
                // Parse DSD and get all Series codes for the indicator, then
                // return an array of Metadata objects.
                const doc = new dom().parseFromString(sdmxDsd)
                const namespaces = xpath.select('/*', doc)[0]._nsMap
                console.log(descriptors)
                const select = xpath.useNamespaces(namespaces)
                const seriesCodeNode = select('//str:Code[@id="' + descriptors['SERIES'] + '"]')
                console.log(seriesCodeNode)
            }
        })
    }
