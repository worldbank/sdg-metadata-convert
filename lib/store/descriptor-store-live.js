...
async function cacheDescriptors() {
    ...
    dsdVersion = select('.//str:DataStructures/str:DataStructure[1]/@version', doc)[0].nodeValue
...
function normalizeSeriesName(name) {
    ...
    // [1.1.1, 1.2.1]
    // Etc...
    const indicatorIdInBrackets = name.match(regex)
    if (indicatorIdInBrackets !== null) {
        const multipleIndicators = indicatorIdInBrackets[1].split(',').length > 1
        if (multipleIndicators) {
            // For the multi-indicator series, we take the approach
            // of leaving the bracketed indicator numbers in place.
            return name
        }
    }
